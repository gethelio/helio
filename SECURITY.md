...
**What the audit trail shows.** Every reload attempt is an audit record (`record_kind: policy_reload`) with the outcome, the hash of the file before and after, the rule and budget counts, the names of the rules that disappeared, and the error on a refusal, written through the immediate queue so its row precedes the first call the new policy serves; the same facts go out on the dashboard event stream as `policy_reload`, and the stderr line stays. Every audit record carries `config_sha256`, the hash of the config bytes in force when it was written, so any decision can be checked against the policy text it ran under. Setting `HELIO_CONFIG_SHA256` to the value `helio config hash` prints makes the proxy refuse a changed file: a reload with a different hash is recorded as `rejected_pinned` and the running policy stays, and a start under a different file exits before it serves anything, which also means a writer of the file can keep the proxy from restarting. The pin holds for the process the operator started; a same-user agent can still restart the proxy without it. Where the agent can write the audit file, the record is deletable after the fact, and the event stream and stderr are the channels that leave the box before that. A file watch the proxy loses is logged and recorded as `watch_failed`; the proxy keeps serving the running policy, retries until it can read the file again, and then reloads it. The hash on every row, checked against `helio config hash`, is the test that does not depend on the watch.
