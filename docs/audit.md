...
| `header_mismatch`            | A standard request header (`Mcp-Method`, `Mcp-Name`, or the `MCP-Protocol-Version` header with its `_meta` mirror) was missing when required or disagreed with the body, and the request was rejected at the inbound door (see [Header Mismatch Rejections](#header-mismatch-rejections)).                 |
