...
MCP 2026-07-28 requires the standard request headers to agree with the JSON-RPC body. Helio enforces this on the streamable-http inbound boundary (issue #226): a request claiming `MCP-Protocol-Version: 2026-07-28` must carry an `Mcp-Method` equal to the body's `method`, an `Mcp-Name` matching the body's name-bearing field on `tools/call`/`prompts/get`/`resources/read`, and a `params._meta` protocol-version mirror equal to the claimed revision — and a present `Mcp-Method` or `Mcp-Name` must agree with the body under any version claim at all. A disagreement is rejected with HTTP 400 and JSON-RPC `-32020` before any policy evaluation, and an immediate audit record is written: a header that lies about the body is a deception attempt, the strongest prior of any rejection class, and the trail must not be blind to exactly the requests that tried to lie. Helio's own policy engine never trusted these headers (it evaluates the parsed body); the enforcement protects header-routing components in front of Helio and the completeness of this trail.
