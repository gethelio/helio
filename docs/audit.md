...
| `protocol_version`     | string \| null | The client's verbatim `MCP-Protocol-Version` wire claim on the inbound request (issue #219). This is the CLIENT'S claim, not the upstream era, and not a validated fact — it is recorded uncapped and exactly as sent. Null when the request carried no header and on records with no MCP wire (drift events, all sideband records).                                         |
