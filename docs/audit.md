...
| `budget_exceeded`            | A named budget blocked the call: an `on_exceed: deny` breach, or a break-glass ticket that was denied or timed out (`approval_status` carries that outcome; budget tickets always fail closed on timeout). The policy decision itself may be `allow`. `evidence_chain.budgets` carries the per-budget snapshots. |
...
When a call matched one or more [named budgets](./policies.md#named-budgets-cross-tool), the record's `evidence_chain.budgets` array carries the money-gate evidence — one block per budget, with the budget's `name`, the call's charge, the pot state the gate saw (`spent`, `limit`, `remaining`, `currency`), and `allowed`. Blocks for committed charges carry `kind` — `spend`, or `approved_overage` for a charge a break-glass approval sanctioned past the limit — mirroring the budget's ledger row; blocks on uncommitted calls carry no `kind`, because nothing was recorded. A contributor that could not read a valid amount records its block with `reason: "invalid_amount"` and no usable amount (the MCP block omits the field; the sideband block carries `attempted_amount: null`). The block shape follows the door that wrote the record: MCP records identify the pot directly (`bucket_key`, plus the attempted `amount` on valid charges), while sideband records reuse the `limits.budgets` wire shape from [`/evaluate`](./adapter-api.md#named-budgets-on-this-api) (`attempted_amount`, `window`, `on_exceed`, `reset_at_ms`).
