...
The sideband door follows the same [`session.on_unresolved`](./configuration.md#session) policy as the MCP door: a call that omits `session_id` while a `key: session` rate limit, spend limit, or budget is engaged is denied under the default `deny` mode — the wire `decision` stays `"deny"`, with `reason` and `feedback` overridden to name the identity strategies that were tried, and the audit record carries `block_reason: "session_unresolved"`. Under `anonymous` such calls pool into the shared `unknown` bucket (pre-0.12 behavior). Dry-run never denies for this: the `dry_run` block reports `would_forward: false`, `limits_ok: false`, and a named `session_unresolved: true` field instead. Under dry-run the same resolution happens as a pure simulation: a matched `rate_limit`/`spend_limit` rule that clears evidence is peeked — never consumed, never reserving sender-key capacity — its snapshot appears in `limits.rate` / `limits.spend`, and `dry_run.would_forward` / `limits_ok` combine the rule limit with the budget peeks (an unreadable spend amount simulates as `limits.spend.reason: "invalid_amount"`). Both paths report `limits` only when the proxy's rate and spend limiters are wired: `helio start` always wires them, so the qualification bites only a direct embedder that constructs `GovernanceService` without one — there, a matched limit rule resolves as an allow (or a would-forward simulation) carrying no snapshot.
