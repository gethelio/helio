...
- **`streamable-http`** (default) — Standard HTTP JSON-RPC. The MCP server exposes an HTTP endpoint. Upstream `text/event-stream` responses are rejected in v0.1 (no passthrough mode).
