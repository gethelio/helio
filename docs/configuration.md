...
| `path`              | string   | No       | `./helio-audit.db` | Path to the SQLite database file. A relative path resolves against the directory `helio start` runs in. Its directory must already exist, and must be writable by the proxy user when the database does not exist yet; Helio does not create it. |
