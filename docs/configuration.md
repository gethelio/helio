...
| `env`              | object   | No          | (none)            | Variables set in the spawned server's environment on top of the proxy's own (stdio only; an entry overrides the proxy's value). Values support `${VAR}` interpolation. Refused when `transport` is not `stdio`.                                                                                                                                                                                                                                                                             |
...
- **`stdio`** — Spawns the MCP server as a child process and communicates over stdin/stdout. Useful for local servers that don't expose an HTTP endpoint.
  The child inherits the proxy's environment and working directory; `env` adds or overrides variables for that child only, and a `${VAR}` in one of its values is resolved from the proxy's environment at load time like every other string.
...
| `command`          | string   | Conditional | —                 | Command to spawn the MCP server. **Required** when `transport` is `stdio`.                                                                                                                                                                     |
| `args`             | string[] | No          | —                 | Arguments passed to the `command` (stdio only).                                                                                                                                                                                                |
| `env`              | object   | No          | (none)            | Variables set in this entry's spawned server on top of the proxy's own environment, as [`upstream.env`](#upstream) (stdio only; refused on other transports).                                                                                  |
| `connect_timeout`  | duration | No          | `10s`             | Timeout for establishing SSE upstream connections.                                                                                                                                                                                             |
| `request_timeout`  | duration | No          | `30s`             | Timeout for upstream HTTP/SSE POST requests.                                                                                                                                                                                                   |
